...
### 3. MERIDIAN as the only normalizer
All other agents are stateless per run. `MERIDIAN` alone owns shared-state normalization in `outputs/state.json`. This keeps concurrency manageable when runs are triggered throughout the day.
...
|----------|-------|--------|
| Which Codex invocation path will production runs use? | Founder / FORGE | Open |
| Which Gmail mailbox or service account will send founder mail? | Founder | Open |
| Which Google Drive folders should map to each artifact class? | Founder / HERALD | Open |
| When should second-wave agents move from disabled to active? | MERIDIAN | Deferred |
